...
import asyncio
import random
from src.utils.ollama_utils import chat_ollama
...
"""
    ...
    response_chat_ollama = await chat_ollama(
        model="llama3.2",
        messages=messages,
        tools=tools,  # No function calling needed here, but included for consistency
    )
    response = {"message": response_chat_ollama}
    ...
    final_response_chat_ollama = await chat_ollama(
        model="llama3.2",
        messages=messages,
        tools=tools,
    )
    final_response = {"message": final_response_chat_ollama}
